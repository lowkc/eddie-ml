...
class elemental_kernel_CV(BaseEstimator):
    ...
    verbose=True, copy_X=True, mae=False, scale_data=False, print_errors=False, normalize_kernel=True):
    ...
    def fit(self, X, y, natoms, atomtypes, show_plot=True, cv_indices=None, random_state=None):
        ...
        print('CV results: gamma={}, lambda={}, kernel_func={}, RMSE={}'.format(gamma, 
    ...
    def predict(self, X, Xatoms, Xatomtypes, verbose=None, variance=False, dist=None):
        '''
...
class elemental_krr_model(BaseEstimator):
    def __init__(self, gamma, lamb, kernel='average', print_errors=True, normalize=True, verbose=True):
        self.gamma = gamma
        self.lamb = lamb
    ...
    def fit(self, X_train, y_train, train_atoms, train_atomtypes, metric='polynomial'):
        self.X_train = X_train
        self.train_atoms = train_atoms
        self.train_atomtypes = train_atomtypes
        t = time.time()
        self.final_kernel = self.kernel(metric=metric, gamma=self.gamma, normalize_kernel=self.normalize)
    ...
    def predict(self, X_test, y_test, test_atoms, test_atomtypes, plot=False, variance=False):
        ...
        if plot:
            ...
            plt.plot(y, '.-', c='orange')
            print('RMSE: ', mean_squared_error(y_test, y, squared=False))
